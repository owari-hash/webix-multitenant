...
export default function ChapterCreateView({ comicId }: Props) {
  const theme = useTheme();
  const [comicTitle, setComicTitle] = useState<string>('');

  useEffect(() => {
    // Fetch comic details to show title
    const fetchComic = async () => {
      try {
        ...
        if (!response.ok) {
          console.error('Failed to fetch comic:', response.status, response.statusText);
          return;
        }
  ...
  return (
    <>
        ...
        <Container>
          <Stack spacing={2}>
            ...
            >
              ...
              <Button
                component={RouterLink}
                href={paths.webtoon.cms.chapters(comicId)}
                color="inherit"
                sx={{ minWidth: 'auto' }}
              >
                Бүлгүүд
            ...
            <Stack direction="row" alignItems="center" spacing={2}>
              ...
              <Box>
                ...
                <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
                  {comicTitle ? `"${comicTitle}" комикт бүлэг нэмэх` : 'Комикт шинэ бүлэг нэмэх'}
                </Typography>
              </Box>
            </Stack>
          </Stack>
      ...
      <Container sx={{ py: { xs: 3, md: 5 } }}>
        <ChapterCreateForm comicId={comicId} comicTitle={comicTitle} />
      </Container>
    </>
  );
}
